...
io.on("connection", (socket) => {
  ...
  socket.on("checkNick", (nickname) => {
    ...
    console.log(Object.values(nickInfo)); // object 값을 배열 형태로 반환

    // ['a','b','c','d'].indexOf('b') : 1
    // ['a','b','c','d'].indexOf('f') : -1
    // 지금 들어온 닉네임과 중복되는 값이 있는지 검사, 존재하지 않을 경우 -1
    if (Object.values(nickInfo).indexOf(nickname) > -1) {
      // 중복된 데이터
      // (1)입장 실패, 닉네임 중복시 에러메세지를 클라이언트에 보낸다.
      socket.emit("error", "이미 존재하는 닉네임입니다.");
    } else {
      nickInfo[socket.id] = nickname; // socketInfo에 nickname정보 넣기 nickInfo = { socket.id: nickname, .., .. }
      // 중복되지 않은 데이터
  ...
  socket.on("send", (msgdata) => {
    ...
    if(msgdata.dm === "all") {
        ...
        // dm, 특정 socket.id를 가진 클라이언트.. 나 포함 X
        io.to(msgdata.dm).emit("message", {
            id: msgdata.myNick, message: msgdata.msg, isDm: true, // isDm은 Dm을 보낼 때 css 작동을 위한 장치
        });

        // 나에게만 데이터 보내주기
        socket.emit("message", {
            id: msgdata.myNick, message: msgdata.msg, isDm: true,
        });
    };
    ...
    socket.on("disconnect", () => {
        ...
        // (1) 객체 변경 후 클라이언트에게 변경된 객체 정보 전달
        io.emit("updateNicks", nickInfo);
        // (2) ~~님이 퇴장하셨습니다. 공고 화면에 띄우기
        io.emit("notice", `${nickInfo[socket.id]}님이 퇴장하셨습니다..`);
        // (3) nickInfo {}에서 특정 키 삭제
